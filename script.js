...
window.addEventListener('load', () => {
    ...
    setTimeout(() => {
        // Simple fade out of loading screen
    ...
    }, 2000); // Changed from 4000 to 2000ms (2 seconds total)
});
...
const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            ...
            if (entry.target.classList.contains('equipment-container')) {
                // Trigger equipment animations with staggered delays
                const equipmentItems = entry.target.querySelectorAll('.camera-section, .lens-section, .audio-section');
...
document.querySelectorAll('.timeline-item, .project-card, .skill-item, .about, .feature-item, .equipment-item, .equipment-container').forEach((el, index) => {
